...
class PerfectHitLauncher3D:
    ...
    def compute_trajectory(
        self, robot_pos, robot_vel, launch_speed, azimuth_deg, elevation_deg, dt=0.01
    ):
        """
    ...
    def add_perfect_hit_scenario(self, robot_pos, robot_vel, label=""):
        ...
        launch_speed, azimuth, elevation, time_of_flight = self.solve_for_perfect_hit(
            robot_pos, robot_vel
        )

        # Compute trajectory
        traj = self.compute_trajectory(
            robot_pos, robot_vel, launch_speed, azimuth, elevation
        )
        ...
        if distance_to_target > 0.01:
            print(
                f"⚠️ Warning: Final position {final_pos} is {distance_to_target:.4f}m from target."
            )
    ...
    def plot_3d(self):
        ...
        for i, traj_data in enumerate(self.trajectories):
            ...
            ax.scatter(
                rx, ry, rz, c=color, s=100, marker="o", edgecolors="black", linewidth=2
            )
            ax.text(
                rx,
                ry,
        ...
        ax.set_title(
            "PERFECT-HIT 3D Ball Trajectories to FIXED Target",
            fontsize=14,
            fontweight="bold",
        )
...
if __name__ == "__main__":
    ...
    print(
        f"🎯 Target is FIXED at: ({FIXED_TARGET[0]:.1f}, {FIXED_TARGET[1]:.1f}, {FIXED_TARGET[2]:.1f})"
    )
    print("-" * 70)
    ...
    print(
        "✅ Distance shown is from final ball position to FIXED TARGET (should be near zero).\n"
    )
    ...
    for i, traj in enumerate(launcher.trajectories):
        ...
        print(f"{traj['label']}:")
        print(
            f"  Robot Start: ({traj['robot_pos'][0]:.1f}, {traj['robot_pos'][1]:.1f}, {traj['robot_pos'][2]:.1f})"
        )
        print(
        ...
        print(
            f"  Final Pos: ({final_pos[0]:.3f}, {final_pos[1]:.3f}, {final_pos[2]:.3f})"
        )
