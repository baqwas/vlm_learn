...
ax.plot(agent_new_position[0], agent_new_position[1], 'k^', markersize=12, label='Agent Current Position')
ax.annotate('Agent New Position (3, 10)', agent_new_position, textcoords="offset points", xytext=(-50, 5), ha='left', fontsize=9, fontweight='bold')

# Visualize the movement (The "two steps left" instruction)
ax.annotate('', xy=agent_new_position, xytext=agent_start_position,
            arrowprops=dict(arrowstyle="->", connectionstyle="arc3,rad=0.0", color='purple', linewidth=2),
            fontsize=8)
