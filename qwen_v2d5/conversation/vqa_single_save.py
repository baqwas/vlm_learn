...
def text_image_generation_local(
    image_folder_path, output_log_file, db_config, platform, user_query
):
    """
            ...
            if file_type in file_types_valid:
                ...
                try:
                    ...
                    messages = [
                        {
                            "role": "user",
                            "content": [{"image": image, "text": user_query}],
                        }
                    ]
                    ...
                    print(
                        f"Error: The file '{file_name}' is not a valid image file. Skipping."
                    )
                except Exception as e:
                    print(
...
if __name__ == "__main__":
    ...
    if os.path.exists(app_config_file_name):
        ...
        if "Settings" in config_app:
            defaults["folder"] = config_app["Settings"].get(
                "folder", defaults["folder"]
            )
            defaults["kpi_log"] = config_app["Settings"].get(
                "kpi_log", defaults["kpi_log"]
            )
            defaults["chat_query"] = config_app["Settings"].get(
                "chat_query", defaults["chat_query"]
            )
        if "App" in config_app:
            defaults["platform"] = config_app["App"].get(
                "platform", defaults["platform"]
            )
    else:
        print(
            f"Warning: Application config file '{app_config_file_name}' not found. Using hardcoded defaults."
        )
    ...
    if os.path.exists(db_config_file_name):
        ...
        if "Database" in config_db:
            defaults["db_host"] = config_db["Database"].get(
                "db_host", defaults["db_host"]
            )
            defaults["db_user"] = config_db["Database"].get(
                "db_user", defaults["db_user"]
            )
            defaults["db_password"] = config_db["Database"].get(
                "db_password", defaults["db_password"]
            )
            defaults["db_name"] = config_db["Database"].get(
                "db_name", defaults["db_name"]
            )
    else:
        print(
            f"Warning: Database config file '{db_config_file_name}' not found. Database operations may fail."
        )
    ...
    parser.add_argument(
        ...
        help="Platform string from config",
    )
    parser.add_argument(
        "--db_host", type=str, default=defaults["db_host"], help="Database host"
    )
    parser.add_argument(
        "--db_user", type=str, default=defaults["db_user"], help="Database user"
    )
    parser.add_argument(
        "--db_password",
        type=str,
    ...
    parser.add_argument(
        "--db_name", type=str, default=defaults["db_name"], help="Database name"
    )
    ...
    text_image_generation_local(
        album, args.kpi_log, db_config, args.platform, defaults["chat_query"]
    )
